...
export class VersionService {
  ...
  public async LoadProjects(
    ...
    const [loaded, availableSolutions] = await Promise.all([
      this.workspaceScanner.ScanProjects(options, previousSelection, lastSelectedProjectId),
      this.workspaceScanner.ListAvailableSolutionPaths()
    ]);
  ...
  public async SelectWorkspaceSolution(): Promise<boolean> {
    return await this.workspaceScanner.SelectSolutionFromWorkspace();
  }

  public async SetUseAllProjects(useAllProjects: boolean): Promise<void> {
    await this.workspaceScanner.SetUseAllProjects(useAllProjects);
  }

  public async SetWorkspaceSolution(solutionPath: string): Promise<void> {
    await this.workspaceScanner.SetWorkspaceSolution(solutionPath);
  }
  ...
  public async UpdateProjects(
    ...
    return {
      ...
      workspaceSettings: refreshed.workspaceSettings,
